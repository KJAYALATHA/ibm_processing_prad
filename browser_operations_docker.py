...
headless_flag = os.environ["HEADLESS"]


def get_driver(browser):
    """
    method to get the type of browser
    :param browser: options are [chrome | chrome_headless]
...
def get_chrome():
    ...
    try:
        ...
        driver = webdriver.Chrome(service=Service(),
                                  options=chrome_options, service_log_path=log_path, desired_capabilities=capabilities)
        stealth(driver,
                languages=["en-US", "en"],
                vendor="Google Inc.",
                platform="Win32",
